...
class Summary:
    ...
    def __call__(self, history, kw):
        ...
        if self.detailed:
            d['rocs']['roc'] = (pd, fa)
            d['hists']['trn_sgn'] = np.histogram(
                y_pred[y_train == 1], bins=bins)
            d['hists']['trn_bkg'] = np.histogram(
                y_pred[y_train != 1], bins=bins)
        ...
        if self.detailed:
            ...
            d['hists']['val_sgn'] = np.histogram(
                y_pred_val[y_val == 1], bins=bins)
            d['hists']['val_bkg'] = np.histogram(
                y_pred_val[y_val != 1], bins=bins)
        ...
        d['max_sp_pd_val'] = (pd[knee], int(
            pd[knee]*sgn_total_val), sgn_total_val)
        d['max_sp_fa_val'] = (fa[knee], int(
            fa[knee]*bkg_total_val), bkg_total_val)
        ...
        if self.detailed:
            d['rocs']['roc_op'] = (pd, fa)
            d['hists']['op_sgn'] = np.histogram(
                y_pred_op[y_op == 1], bins=bins)
            d['hists']['op_bkg'] = np.histogram(
                y_pred_op[y_op != 1], bins=bins)
...
class Evaluator:
    ...
    def __init__(self, eval_file_name: str,
        ...
        if self.ringer_version == 'v1':
            ...
            # EM1 list
            sum_rings = 8
            em1 = [prefix % iring for iring in range(
                sum_rings, sum_rings+(64//2))]

            # EM2 list
            sum_rings = 8+64
            em2 = [prefix % iring for iring in range(
                sum_rings, sum_rings+(8//2))]

            # EM3 list
            sum_rings = 8+64+8
            em3 = [prefix % iring for iring in range(
                sum_rings, sum_rings+(8//2))]

            # HAD1 list
            sum_rings = 8+64+8+8
            had1 = [prefix % iring for iring in range(
                sum_rings, sum_rings+(4//2))]

            # HAD2 list
            sum_rings = 8+64+8+8+4
            had2 = [prefix % iring for iring in range(
                sum_rings, sum_rings+(4//2))]

            # HAD3 list
            sum_rings = 8+64+8+8+4+4
            had3 = [prefix % iring for iring in range(
                sum_rings, sum_rings+(4//2))]
