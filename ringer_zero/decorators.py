...
class Summary:
    ...
    def __call__(self, history, kw):
        ...
        logger.info(
            "Train samples     : Prob. det (%1.4f), False Alarm (%1.4f), SP (%1.4f), AUC (%1.4f) and MSE (%1.4f)",
            pd[knee],
            fa[knee],
            sp[knee],
            d["auc"],
        ...
        logger.info(
            "Validation Samples: Prob. det (%1.4f), False Alarm (%1.4f), SP (%1.4f), AUC (%1.4f) and MSE (%1.4f)",
            pd[knee],
            fa[knee],
            sp[knee],
            d["auc_val"],
        ...
        logger.info(
            "op Samples : Prob. det (%1.4f), False Alarm (%1.4f), SP (%1.4f), AUC (%1.4f) and MSE (%1.4f)",
            pd[knee],
            fa[knee],
            sp[knee],
            d["auc_val"],
        ...
        d["max_sp_pd_op"] = (
            pd[knee],
            int(pd[knee] * (sgn_total + sgn_total_val)),
            (sgn_total + sgn_total_val),
        )
        d["max_sp_fa_op"] = (
            fa[knee],
            int(fa[knee] * (bkg_total + bkg_total_val)),
            (bkg_total + bkg_total_val),
        )
        d["max_sp_op"] = sp[knee]
...
class Reference:
    ...
    def __init__(self, refs):
        ...
        for key, ref in refs.items():
            ...
            logger.info(
                "%s (pd=%1.2f, fa=%1.2f, sp=%1.2f)",
                key,
                pd[0] * 100,
                fa[0] * 100,
                sp_func(pd[0], fa[0]) * 100,
            )
    ...
    def __call__(self, history, kw):
        ...
        sp_val = np.sqrt(
            np.sqrt(pd_val * (1 - fa_val)) * (0.5 * (pd_val + (1 - fa_val)))
        )
        ...
        for key, ref in self.__references.items():
            ...
            logger.info("          : %s", key)
            logger.info(
                "Reference : [Pd: %1.4f] , Fa: %1.4f and SP: %1.4f ",
                ref["pd"][0] * 100,
                ref["fa"][0] * 100,
                ref["sp"] * 100,
            )
            logger.info(
                "Train     : [Pd: %1.4f] , Fa: %1.4f and SP: %1.4f ",
                d["pd"][0] * 100,
                d["fa"][0] * 100,
                d["sp"] * 100,
            )
            logger.info(
                "Validation: [Pd: %1.4f] , Fa: %1.4f and SP: %1.4f ",
                d["pd_val"][0] * 100,
                d["fa_val"][0] * 100,
                d["sp_val"] * 100,
            )
            logger.info(
                "Operation : [Pd: %1.4f] , Fa: %1.4f and SP: %1.4f ",
                d["pd_op"][0] * 100,
                d["fa_op"][0] * 100,
                d["sp_op"] * 100,
            )
...
class Evaluator:
    ...
    def __call__(self, history, kw):
        ...
        if self.detailed:
            ...
            d["hists_eval"]["trn_sgn"] = np.histogram(y_pred[y_eval == 1], bins=bins)
            d["hists_eval"]["trn_bkg"] = np.histogram(y_pred[y_eval != 1], bins=bins)

        logger.info(
            "Evaluation samples     : Prob. det (%1.4f), False Alarm (%1.4f), SP (%1.4f), AUC (%1.4f) and MSE (%1.4f)",
            pd[knee],
            fa[knee],
            sp[knee],
            d["auc_eval"],
